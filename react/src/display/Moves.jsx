...
function Moves({ moves, api }) {
    let items = [];

    if (moves != null) {
        for (const [key, value] of Array.from(moves.entries())) {
            let name = value.slice(1, value.indexOf(':') + 1);
            let rest = value.slice(value.indexOf(':') + 1).trim();
            let sp = rest.split(' ');
            let img_number = key
            if (parseInt(api) < 3) { img_number = key + 1; }
            ...
            if (sp.length === 3) { // move, name, [-|--|(challenge)|(unknown)], [], points, score 
                ...
                let marker = '';
                if (isScrabble(sp[1])) { marker = 'table-warning'; }
                items.push(
