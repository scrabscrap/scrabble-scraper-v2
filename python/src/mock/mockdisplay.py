...
class MockDisplay(Display):
    ...
    def add_time(self, player, t1, p1, t2, p2):
        ...
        left = f'{doubt1} -{m1:1d}:{s1:02d} ({p1:4d})' if config.MAX_TIME - t1 < 0 else f'{doubt1} {m1:02d}:{s1:02d} ({p1:4d})'
        right = f'{doubt2} -{m2:1d}:{s2:02d} ({p2:4d})' if config.MAX_TIME - t2 < 0 else f'{doubt2} {m2:02d}:{s2:02d} ({p2:4d})'
