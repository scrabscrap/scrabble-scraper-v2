...
class Custom2020PILBoard(CustomBoard):
    """Implementation custom 2012 scrabble board analysis with Pillow (uses PIL.quantize)"""

    # Pillow uses hsv_full colors
    FIELD_COLOR = ([105, 5, 5], [180, 255, 255])
    TLETTER_COLOR0 = ([210, 80, 10], [255, 255, 255])
    TLETTER_COLOR1 = ([0, 80, 10], [10, 255, 255])
    DLETTER_COLOR = ([0, 10, 0], [255, 255, 80])
    TWORD_COLOR = ([15, 80, 50], [70, 255, 255])
    DWORD_COLOR = ([0, 160, 60], [60, 255, 255])
    ...
    def filter_image(cls, _image: MatLike) -> tuple[Optional[MatLike], set]:
        ...
        def is_tile(coord: tuple[int, int], hsv: tuple[int, int, int], rgb: tuple[int, int, int]) -> bool:
            ...
            if coord in TRIPLE_WORDS:  # dark red
                return not between(hsv, *cls.TWORD_COLOR)
            if coord in DOUBLE_WORDS:  # light red
                return not between(hsv, *cls.DWORD_COLOR)
            if coord in TRIPLE_LETTER:  # dark blue
                return not (between(hsv, *cls.TLETTER_COLOR0) or between(hsv, *cls.TLETTER_COLOR1))
        ...
        for col in range(15):
            for row in range(15):
                ...
                color_rgb = sorted(pil_rgb.getcolors(), reverse=True)[0][1]  # type: ignore[arg-type]
                rgb_table[(col, row)] = color_rgb

                pil_hsv = pil_img.convert(mode='HSV')
                color_hsv_count, color_hsv = sorted(pil_hsv.getcolors(), reverse=True)[0]  # type: ignore[arg-type]
...
def main():  # pylint: disable=too-many-locals
    ...
    for fn in files:
        image = cv2.imread(fn)
        warped = Custom2020PILBoard.warp(image)

        result, tiles_candidates = Custom2020PILBoard.filter_image(_image=warped.copy())
