...
pip install -U pip setuptools wheel -r requirements.txt --upgrade-strategy=eager
...
if [[ $(/usr/bin/node -v) == *"v"${NODE_MAJOR}* ]]; then
    echo "node ${NODE_MAJOR} is installed, skipping..."
else
